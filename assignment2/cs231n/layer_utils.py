...
def affine_batchnorm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    - cache: Opject to give the backward pass
    """
    a, fc_cache = affine_forward(x, w, b)
    y, batchnorm_cache = batchnorm_forward(a, gamma, beta, bn_param)
...
def affine_batchnorm_relu_backward(dout, cache):
    ...
    da, dgamma, dbeta = batchnorm_backward(dy, batchnorm_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def affine_layernorm_relu_forward(x, w, b, gamma, beta, ln_param):
    """"""
    a, fc_cache = affine_forward(x, w, b)
    y, layernorm_cache = layernorm_forward(a, gamma, beta, ln_param)
    out, relu_cache = relu_forward(y)
...
def affine_layernorm_relu_backward(dout, cache):
    ...
    dy = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = layernorm_backward(dy, layernorm_cache)
    dx, dw, db = affine_backward(da, fc_cache)
...
def affine_relu_dropout_backward(dout, cache):
    ...
    dy = dropout_backward(dout, dropout_cache)
...
def affine_batchnorm_relu_dropout_backward(dout, cache):
    ...
    dy = dropout_backward(dout, dropout_cache)
    dy = relu_backward(dy, relu_cache)
    dy, dgamma, dbeta = batchnorm_backward(dy, batchnorm_cache)
    dx, dw, db = affine_backward(dy, fc_cache)
